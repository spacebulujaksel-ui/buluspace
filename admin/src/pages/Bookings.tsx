      ...
      <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden">
        {loading ? (
          ...
          <div className="md:hidden divide-y divide-neutral-100">
            {bookings.map((apt) => (
              <div key={apt.id} className="px-4 py-3.5 space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-xs font-mono font-semibold text-neutral-800">{apt.booking_code}</p>
                    <p className="text-[13px] font-medium text-neutral-900 mt-0.5">{apt.customer_name}</p>
                    {apt.customer_gender && (
                      <span
                        className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold mt-1 ${
                          apt.customer_gender === 'Pria'
                            ? 'bg-sky-50 text-sky-700 border border-sky-200'
                            : 'bg-pink-50 text-pink-700 border border-pink-200'
                        }`}
                  ...
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium shrink-0 ${
                      STATUS_STYLES[apt.status] ?? 'bg-neutral-100 text-neutral-500'
                    }`}
                  >
                    <span className="w-1.5 h-1.5 rounded-full bg-current mr-1.5 opacity-50" />
                    {apt.status}
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[11px] text-neutral-500">
                  <span>Tanggal: {formatDate(apt.appointment_date)}</span>
                  <span>Jam: {apt.start_time.slice(0, 5)}–{apt.end_time.slice(0, 5)}</span>
                  <span className="col-span-2">Terapis: {apt.therapist?.name ?? '—'}</span>
                </div>
                <div className="text-[11px] text-neutral-600">
                  {apt.details?.length
                    ? apt.details.map((d) => `${d.service?.name ?? `Layanan #${d.service_id}`} × ${d.quantity}`).join(', ')
                    : '—'}
                </div>
                <div className="flex items-center justify-between gap-2 pt-1 border-t border-neutral-100">
                  <span className="text-[13px] font-medium font-mono text-neutral-900">{formatRupiah(Number(apt.total_price))}</span>
                  <div className="flex items-center gap-1">
                    {apt.status === 'Confirmed' && (
                      <button
                        onClick={() => updateStatus(apt.id, 'Completed')}
